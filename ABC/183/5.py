...
for row in range(H - 2, -1, -1):
    for column in range(W - 2, -1, -1):
        if stage[row][column] == '#':
            continue
        if stage[row + 1][column] == '.':
            dp[row][column] += dp_top[row + 1][column]
            dp[row][column] %= MOD
        if stage[row][column + 1] == '.':
            dp[row][column] += dp_left[row][column + 1]
            dp[row][column] %= MOD
        if stage[row + 1][column + 1] == '.':
            dp[row][column] += dp_top_left[row + 1][column + 1]
            dp[row][column] %= MOD

        if stage[row + 1][column] == '.':
            dp_top[row][column] += dp_top[row + 1][column] + dp[row][column]
        else:
            dp_top[row][column] += dp[row][column]
        dp_top[row][column] %= MOD
        if stage[row][column + 1] == '.':
            dp_left[row][column] += dp_left[row][column + 1] + dp[row][column]
        else:
            dp_left[row][column] += dp[row][column]
        dp_left[row][column] %= MOD
        if stage[row + 1][column + 1] == '.':
            dp_top_left[row][column] += dp_top_left[row + 1][column + 1] + dp[row][column]
        else:
            dp_top_left[row][column] += dp[row][column]
